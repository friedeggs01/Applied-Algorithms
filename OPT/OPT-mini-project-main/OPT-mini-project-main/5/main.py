...
class Individual:
	def __init__(self, N, K, demand, distance_matrix: list[list], chromosome = None):
	...
	def calc_fitness(self):
		...
		for gene in self.chromosome:
			...
			if gene <= 0:		
				#truck always end at index 0
				self.Routes[index].append(0)	
				index += 1
				#truck always start at index 0
				self.Routes[index].append(0)
			else:
		...
		for i, route in enumerate(self.Routes):
			for j in range(1, len(route)):
				fitnesses[i] += self.distance_matrix[route[j-1]][route[j]] + self.demand[route[j]]
...
#Solution class	
class GA:
	def __init__(self, N, K, demand, distance_matrix: list[list], n, generations, mutation_rate):
		#Populations contain n Individual
		self.populations: list[Individual] = [Individual(N, K, demand, distance_matrix) for _ in range(n)]
		self.n = n
	...
	def solve(self):
		...
		for generation in range(self.generations):
			...
			for i in range(self.n):
				...
				child = Individual(self.N, self.K, self.demand, self.distance_matrix, child_chromosome)
...
def main():
	try:
		N, K, demand, distance_matrix = import_data('5/test.txt')
	except:
		N, K, demand, distance_matrix = import_data('test.txt')

	populations_num = 100
	generations = 100
	mutation_rate = 0.1

	sol = GA(N, K, demand, distance_matrix, populations_num, generations, mutation_rate)
